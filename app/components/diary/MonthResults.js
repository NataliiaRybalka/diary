...
function MonthResults() {
    ...
    const getMonthResult = async () => {
        ...
        const res = await fetch(
            `${SERVER}/diary/result/${JSON.parse(user).id}/${month}`
        );
                    ...
                    tableData.map((row, rowI) => (
                        <View style={styles.rowTable} key={rowI}>
                            ...
                            <Text
                                ...
                                key={`${uuid.v4()}${rowI}`}
                            >
                                {tableData[rowI].menstrualDay}
                            </Text>
                            <Text
                                style={styles.cell}
                                key={`${uuid.v4()}${rowI}`}
                            >
                                {tableData[rowI].totalHours}
                            </Text>
                            <Text
                                style={styles.cell}
                                key={`${uuid.v4()}${rowI}`}
                            >
                                {tableData[rowI].physicalActivity}
                            </Text>
                            <Text
                                style={styles.cell}
                                key={`${uuid.v4()}${rowI}`}
                            >
                                {tableData[rowI].drankWater}
                            </Text>
                        </View>
                    ))
                ) : (
