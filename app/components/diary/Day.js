...
function Day() {
    ...
    const getPage = async () => {
        ...
        const res = await fetch(
            `${SERVER}/diary/page/${JSON.parse(user).id}/${chosenDate}`
        );
    ...
    return (
        <ScrollView
            style={[
                ...
                showPicker && { backgroundColor: "rgba(0, 0, 0, 0.7)" },
            ]}
        >
            <DayPicker
                day={chosenDate}
                setDay={setChosenDate}
            ...
            <View style={{ marginHorizontal: 5 }}>
                ...
                <View style={styles.div}>
                    <Text style={styles.label}>
                        {t("Day of the menstrual cycle")}
                    </Text>
                    <TextInput
                        style={styles.inputNum}
                        value={
                            pageData.menstrualDay
                                ? String(pageData.menstrualDay)
                                : ""
                        }
                        keyboardType="number-pad"
                        onChangeText={(text) =>
                            onChangeInput(text, "menstrualDay")
                        }
                    />
                </View>
                ...
                <View style={styles.div}>
                    <Text style={styles.label}>
                        {t("Total hours of sleep per day")}
                    </Text>
                    <TextInput
                        style={styles.inputNum}
                        value={
                            pageData.totalHours
                                ? String(pageData.totalHours)
                                : ""
                        }
                        keyboardType="number-pad"
                        onChangeText={(text) =>
                            onChangeInput(text, "totalHours")
                        }
                    />
                </View>
                ...
                <View style={styles.div}>
                    ...
                    <TextInput
                        ...
                        value={
                            pageData.drankWater
                                ? String(pageData.drankWater)
                                : ""
                        }
                        keyboardType="number-pad"
                        onChangeText={(text) =>
                            onChangeInput(text, "drankWater")
                        }
                    />
                </View>
                ...
                <View style={styles.div}>
                    ...
                    <TextInput
                        style={styles.inputNum}
                        value={
                            pageData.physicalActivity
                                ? String(pageData.physicalActivity)
                                : ""
                        }
                        keyboardType="number-pad"
                        onChangeText={(text) =>
                            onChangeInput(text, "physicalActivity")
                        }
                    />
                </View>
            ...
            <View style={styles.btn}>
                <Text style={styles.btnText} onPress={savePageData}>
                    {t("Save")}
                </Text>
            </View>

            {saved && (
                <Text style={styles.result}>{t("Saved successfully")}</Text>
            )}
        </ScrollView>
    );
}
