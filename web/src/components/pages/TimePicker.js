...
function TimePicker({ time, setTime, setShowPicker, pickerTitle = null }) {
    ...
    return (
        <div
            className={
                pickerTitle === "day"
                    ? "pickerContainer timePickerContainer dayTimePickerContainer"
                    : "pickerContainer timePickerContainer"
            }
        >
                ...
                <div className="hourContainer">
                    {[...Array(24)].map((hour, hourNum) => (
                        <span
                            ...
                            style={{
                                backgroundColor:
                                    hourNum === Number(selectedHour) &&
                                    bgColour,
                            }}
                            onClick={() =>
                                setSelectedHour(
                                    String(hourNum).length === 1
                                        ? `0${hourNum}`
                                        : hourNum
                                )
                            }
                ...
                <div className="hourContainer">
                    {[...Array(60)].map((minute, minuteNum) => (
                        <span
                            ...
                            style={{
                                backgroundColor:
                                    minuteNum === Number(selectedMinute) &&
                                    bgColour,
                            }}
                            onClick={() =>
                                onChange(
                                    String(minuteNum).length === 1
                                        ? `0${minuteNum}`
                                        : minuteNum
                                )
                            }
            ...
            <span className="cancel" onClick={() => setShowPicker(false)}>
                {t("CANCEL")}
            </span>
        </div>
    );
