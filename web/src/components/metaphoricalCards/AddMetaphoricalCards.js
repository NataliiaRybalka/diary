    ...
    const onChangeInput = async (e, cardI) => {
        if (e.target.name === "file") {
            ...
            const resp = await axios.post(
                `${SERVER}/metaphorical-cards/file`,
                formData,
                {
                    headers: {
                        "Content-Type":
    ...
    return (
        <div className="adminPanel">
            ...
            <div className="adminPanelDiv">
                <select
                    className="adminSelectDeck"
                    style={{ backgroundColor: bgColour }}
                    value={deck}
                    onChange={onHandleChangeDeck}
                >
                    <option value={FULCRUM}>{t("Fulcrum")}</option>
                    <option value={INTERNAL_COMPASS}>
                        {t("Internal Compass")}
                    </option>
                </select>
                <br />
                ...
                <table className="menstrualCycleTable adminCardTable">
                    <thead>
                        <tr>
                            ...
                            <td>
                                <span>{t("Russian Description")}</span>
                            </td>
                            <td>
                                <span>{t("Ukrainian Description")}</span>
                            </td>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr key={cardI}>
                                <td className="adminCardTableFile">
                                    <input
                                        ...
                                        name="file"
                                        onChange={(e) =>
                                            onChangeInput(e, cardI)
                                        }
                                    />
                                    {card && card.file && (
                                        <img
                                            src={card.fileUrl}
                                            alt={card.fileUrl.name}
                ...
                <button className="submit savePage" onClick={saveCards}>
                    {t("Save")}
                </button>
            </div>
        </div>
