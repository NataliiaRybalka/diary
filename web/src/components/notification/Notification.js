...
function Notification({ user }) {
    ...
    const getNotificationSettings = async () => {
        ...
        for (const notif in data) {
            ...
            if (timeForSend[0] - timezone < 24)
                timeForSend[0] = timeForSend[0] - timezone;
            else timeForSend[0] = timeForSend[0] - timezone - 24;

            timeForSend[0] = String(timeForSend[0]);
            timeForSend[0] =
                timeForSend[0].length === 1
                    ? `0${timeForSend[0]}`
                    : timeForSend[0];
            ...
            <div className="form">
                <div className="checkboxDelete">
                    ...
                    <input
                        type="checkbox"
                        name="morning"
                        checked={notifications.morning?.send}
                    ...
                    <div className="pickerDivTime">
                        {showMorningTimePicker ? (
                            <TimePicker
                                ...
                                setShowPicker={setShowMorningTimePicker}
                            />
                        ) : (
                            <div
                                onClick={() =>
                            ...
                            >
                                {morningTime
                                    ? morningTime
                                    : notifications.morning.time}
                            </div>
                        )}
                    </div>
                    <select
                        className="langNotifications"
                        name="morning"
                        value={notifications.morning?.language}
                        onChange={(e) => onChangeInput(e)}
                    >
                ...
                <div className="checkboxDelete">
                    ...
                    <input
                        type="checkbox"
                        name="evening"
                        checked={notifications.evening?.send}
                        onChange={(e) => onChangeInput(e)}
                    />
                    <div className="pickerDivTime">
                        {showEveningTimePicker ? (
                            <TimePicker
                                ...
                                setShowPicker={setShowEveningTimePicker}
                            />
                        ) : (
                            <div
                                onClick={() =>
                    ...
                    <select
                        className="langNotifications"
                        name="evening"
                        value={notifications.evening?.language}
                        onChange={(e) => onChangeInput(e)}
                    >
                        <option value="en">en</option>
                        <option value="ru">ru</option>
                ...
                <div className="checkboxDelete">
                    ...
                    <input
                        type="checkbox"
                        name="day_plan"
                        checked={notifications.day_plan?.send}
                        onChange={(e) => onChangeInput(e)}
                    />
                </div>
                {err && <p className="pError">{err}</p>}
                <button
                    className="submit restoreSubmit notificationSubmit"
