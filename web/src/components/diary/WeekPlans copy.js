...
function WeekPlans() {
	...
	const [updatedDayNum, setUpdatedDayNum] = useState(null);
						...
						<div>
							...
							{[...Array(rows[dayNum])].map((row, rowNumber) => (
								<div className='inputs' name={engDates[dayNum]} key={rowNumber} >
									...
									<div>
										{(showPicker && updatedRow === rowNumber && updatedDayNum === dayNum)
											? <TimePicker time={time} setTime={setTime} setShowPicker={setShowPicker} />
											: <div
												onClick={() => {
													setUpdatedDayNum(dayNum);
													setUpdatedRow(rowNumber);
													setShowPicker(!showPicker)
												}}
											>
												{(time && updatedRow === rowNumber && updatedDayNum === dayNum)
													? time
