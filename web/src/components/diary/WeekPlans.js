...
function WeekPlans() {
    ...
    const handleRows = (dayNum, type) => {
        if (type === "+") {
            ...
            if (!dayPlan)
                newWeekPlan[dayNum] = { plans: [{ time: "00:00", plan: "" }] };
            else if (!dayPlan.plans)
                dayPlan.plans = [{ time: "00:00", plan: "" }];
            else
                dayPlan.plans = [...dayPlan.plans, { time: "00:00", plan: "" }];
    ...
    const save = async (dayNum) => {
        ...
        const endpoint = dayPlan._id
            ? `week-plan/${dayPlan._id}`
            : `day-plan/${JSON.parse(localStorage.getItem("user")).id}`;
                            ...
                            {[...Array(rows[dayNum])].map((row, rowNumber) => (
                                <div className="inputs" key={rowNumber}>
                                    ...
                                    <input
                                        ...
                                        className="planInput"
                                        value={
                                            weekPlan[dayNum]?.plans[rowNumber]
                                                ?.plan
                                        }
                                        onChange={(e) =>
                                            onUpdateInput(e, rowNumber, dayNum)
                                        }
                                    />
                                </div>
                            ))}
