    ...
    try {
        ...
        for (const card of cards) {
            if (card.deck === FULCRUM) {
                ...
                promises.push(
                    await InternalCompassSchema.updateOne(
                        { _id: card.file },
                        {
                            descriptionEn: card.descriptionEn,
                            descriptionRu: card.descriptionRu,
...
export const postCardFile = async (req: Request, res: Response) => {
    ...
    try {
        let card;
        if (deck === FULCRUM)
            card = await FulcrumSchema.create({ file: file.path });
        else if (deck === INTERNAL_COMPASS)
            card = await InternalCompassSchema.create({ file: file });
...
export const getDeckCards = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (deck === FULCRUM) cards = await FulcrumSchema.find();
            ...
            cards = await InternalCompassSchema.find();
...
export const getCard = async (req: Request, res: Response) => {
    ...
    try {
        let card;
        if (deck === FULCRUM)
            card = await FulcrumSchema.aggregate([{ $sample: { size: 1 } }]);
        else if (deck === INTERNAL_COMPASS)
            card = await InternalCompassSchema.aggregate([
                { $sample: { size: 1 } },
            ]);
