...
const saveCardsToDb = async () => {
    try {
        const filesPathes = [
            path.join(__dirname, "lib/files/fulcrum.csv"),
            path.join(__dirname, "lib/files/internalCompass.csv"),
        ];
                ...
                fs.createReadStream(filePath).pipe(
                    csv
                        ...
                        .on("data", async (row) => {
                            ...
                            if (deck === "fulcrum")
                                await FulcrumSchema.create({ ...row });
                            else if (deck === "internalCompass")
                                await InternalCompassSchema.create({ ...row });
                        })
