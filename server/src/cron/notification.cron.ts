...
export const job = new CronJob("*/10 * * * *", async () => {
    ...
    try {
        ...
        if (timeArr[1] - 10 >= 0) {
            ...
            timeArr[1] =
                String(timeArr[1]).length === 1 ? `0${timeArr[1]}` : timeArr[1];
        } else {
            if (timeArr[0] - 1 >= 0) {
                timeArr[0] = timeArr[0] - 1;
                timeArr[1] = 60 + (timeArr[1] - 10);
                timeArr[1] =
                    String(timeArr[1]).length === 1
                        ? `0${timeArr[1]}`
                        : timeArr[1];
            } else {
        ...
        endTimeArr[0] =
            endTimeArr[0][0] === "0" ? endTimeArr[0][1] : endTimeArr[0];
        ...
        notifications = notifications.filter(
            (notif) => notif.date === taskDate || notif.date === "everyday"
        );
            ...
            if (
                ...
                promises.push(
                    sendMail(
                        ...
                        notification.type,
                        {
                            username: notification.userData.username,
                            link: `${WEB}/my-diary/diary`,
                        },
